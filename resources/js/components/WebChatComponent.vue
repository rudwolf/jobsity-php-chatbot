<template>
    <div class="ChatLog_wrapper">
        <div class="arrow"></div>
        <ul class="ChatLog" id="ChatBotLog">
            <li class="ChatLog__entry" v-for="message in messages" :class="{'ChatLog__entry_mine': message.isMine}">
                <img class="ChatLog__avatar" src="/bot-avatar.png" />
        ...
        <div class="ChatLog_controls">
            <input type="file" class="ChatAttachment" id="attachment" value="Attachment" />
            <input type="text" class="ChatInput" id="ChatMsg" @keyup.enter="sendMessage" v-model="newMessage" placeholder="Please type 'bank' without quotes, to start!">
        </div>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            sendMessage() {
                ...
                this.callAPI(messageText, false, null, function() {
                    var objLog = document.getElementById("ChatBotLog");
                    setTimeout(function(){ objLog.scrollTop = objLog.scrollHeight; }, 500);
                });
            }
        }
    }
